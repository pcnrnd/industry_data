...
#################### LabelEncoding     
        ...
        if data_for_labelencoding:                                                              # 데이터프레임에 str 타입이 있는 경우, int 타입으로 정수 인코딩
            ...
            if updated_df is not None:
                ...
                if label_col_name == target_feature:
                    updated_df[target_feature] = le.fit_transform(updated_df[target_feature])
                    # print(updated_df[target_feature])
...
#################### sampling strategy
                ...
                with st.spinner('Wait for it...'):
                    ...
                    if feature_selection:
                        if feature_selection[0] == 'Multiclass':
                            # Multiclass: 실제 데이터 기준으로 sampling_strategy 생성
                            # target_col = target_feature[0] if isinstance(target_feature, list) else target_feature
                            

                            # unique_labels = updated_df[target_col].unique()
                            # print(unique_labels)
                            unique_labels = updated_df[target_feature].unique()
                            print("==========", unique_labels)
                            print(unique_labels)
                            sampling_strategy = {int(label): round(thresh) for label in unique_labels}
                            print(f"Multiclass sampling_strategy: {sampling_strategy}")

                        elif feature_selection[0] == 'Binary class':
                            sampling_strategy = thresh
                            print(f"Binary class sampling_strategy: {sampling_strategy}")

                    # 백엔드 API는 사용하지 않음 (데이터 불일치 문제로 인해)
    ...
    #################### Oversampling Start
                    ...
                    X_over_resampled, y_over_resampled = SMOTE(sampling_strategy=sampling_strategy).fit_resample(updated_df.drop(target_feature, axis=1), updated_df[target_feature])          # sampling_strategy 옵션: 'auto', sampling_strategy
