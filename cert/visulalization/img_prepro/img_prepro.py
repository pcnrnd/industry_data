...
if uploaded_files:
    ...
    if image_file_paths:
        ...
        # 저장 옵션
        save_format = st.sidebar.selectbox("저장 형식", ["PNG", "JPG", "원본과 동일"])
        ...
        if st.sidebar.button("⚡ 전체 이미지 일괄 처리 및 저장", type="primary", key="batch_process", width='stretch'):
            # 이전 결과 초기화 (방법 2)
            ...
            with st.spinner("이미지 처리 중..."):
                try:
                    ...
                    request_data = {
                        ...
                        "save_format": save_format  # 저장 형식 전달
                    }
                    
                    response = requests.post(f"http://localhost:8000/api/batch_preprocessing", json=request_data, stream=True)
                                ...
                                if line_str.startswith('data: '):
                                    try:
                                        ...
                                        if data.get('status') == 'completed':
                                            ...
                                            failed_count += 1
                                            st.error(f"처리 실패: {os.path.basename(data['current_file'])} - {data.get('error', 'Unknown error')}")
                                            
                                    except json.JSONDecodeError:
                                        continue
                    else:
                        st.error(f"배치 처리 요청 실패: {response.status_code} - {response.text}")
                        failed_count = len(image_file_paths)
