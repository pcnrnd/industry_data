...
if uploaded_file is not None:
    ...
    with st.container():
        st.title("📊 데이터 분포 분석 및 시각화")
        st.info("컬럼 타입이 잘못 분류된 경우, 수치형, 범주형 컬럼을 직접 선택해 주세요.")
        st.dataframe(pd.DataFrame({'컬럼명': data.columns, '타입': data.dtypes.astype(str)}))
        manual_numeric = st.sidebar.multiselect("수치형(연속형) 컬럼 직접 선택", data.columns, default=numeric_cols)
        manual_categorical = st.sidebar.multiselect("범주형(카테고리) 컬럼 직접 선택", data.columns, default=categorical_cols)
        ...
        if st.sidebar.button("데이터 분석 시작", width='stretch'):
            ...
            with st.spinner("상세 데이터 분석 중..."):
                # 분석 실행
                request_data = json.dumps({"data": data.to_dict(orient='records'), "numeric_cols": numeric_cols, "categorical_cols": categorical_cols})
                response = requests.post(f"http://localhost:8000/api/analysis", json=request_data)
                result = response.json()
                missing_ratio, duplicate_ratio, quality_issues = result['missing_ratio'], result['duplicate_ratio'], result['quality_issues']
                outlier_info = result['outlier_info']
                distribution_insights = result['distribution_insights']
                ...
                with st.expander("🔍 데이터 품질 분석", expanded=True):
                    ...
                    with col1:
                        # 결측값 비율 게이지 차트
                        import plotly.graph_objects as go
                        fig_missing = go.Figure(go.Indicator(
                            mode = "gauge+number",
                            value = missing_ratio,
                            title = {'text': "결측값 비율 (%)"},
                    ...
                    with col2:
                        # 중복값 비율 게이지 차트
                        fig_duplicate = go.Figure(go.Indicator(
                            mode = "gauge+number",
                        ...
                        fig_duplicate.update_layout(height=300)
                        st.plotly_chart(fig_duplicate, use_container_width=True)
                ...
                with st.expander("🔍 이상값 분석", expanded=True):
                    if outlier_info:
                        ...
                        for outlier in outlier_info:
                            ...
                            count_part = parts[1].strip().split('개')[0]
                            ratio_part = parts[1].strip().split('(')[1].split('%')[0]
                            
                            outlier_cols.append(col_name)
                            outlier_counts.append(int(count_part))
                            outlier_ratios.append(float(ratio_part))
                        
                        # 막대그래프로 시각화
                        fig_outlier = px.bar(x=outlier_cols, y=outlier_counts,
                                            title="컬럼별 이상값 개수",
                                            labels={'x': '컬럼명', 'y': '이상값 개수'},
